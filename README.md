## Hi there 👋

<!--
**himanshudev28/himanshudev28** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...# Hi 👋, I'm Himanshu Singh

### Full Stack Developer | React & Next.js Developer | Java Programmer

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=himanshudev28&label=Profile%20Views&color=0e75b6&style=flat" alt="profile views" />
</p>

---

## 🚀 About Me

I'm a B.Tech student and Full Stack Developer passionate about building scalable web applications and solving real-world problems through technology.

* 🎓 B.Tech @ Invertis University (CGPA: 8.5)
* 💻 Full Stack Web Developer
* ⚛️ React.js & Next.js Developer
* ☕ Java Programmer
* 🌱 Currently exploring Backend Development & System Design
* 📚 Regularly solving DSA problems
* 🚀 Building production-ready projects

---

## 🛠️ Tech Stack

### Languages

```text
Java • JavaScript • TypeScript • HTML • CSS • Python (Basic) • C++ (Basic)
```

### Frontend

```text
React.js • Next.js • Tailwind CSS • Bootstrap • ShadCN UI
```

### Backend

```text
Node.js • Express.js • REST APIs
```

### Database & BaaS

```text
MongoDB • PostgreSQL • Supabase
```

### Tools

```text
Git • GitHub • VS Code • Postman • Tableau • Power BI
```

---

## 💼 Experience

### Frontend Web Developer Intern @ Elevatifier Pvt Ltd

* Built responsive interfaces using React.js and Tailwind CSS
* Integrated APIs and improved user experience
* Collaborated using GitHub and modern workflows

### AI Azure Intern @ Microsoft + Edunet Foundation

* Learned Azure AI Services
* Worked with Machine Learning fundamentals
* Explored cloud-based AI solutions

### Full Stack Developer Intern @ EduSkills

* Developed responsive web applications
* Worked with HTML, CSS, JavaScript
* Completed real-world development assignments

---

## 🚀 Featured Projects

### 🎟️ SeatSync

A Full Stack Event Ticket Booking Platform.

**Tech Stack**

```text
React • TypeScript • Node.js • MongoDB • Tailwind CSS
```

**Features**

* Interactive seat booking
* Google OAuth Authentication
* JWT Authentication
* QR Ticket Generation
* Admin Dashboard
* Revenue Analytics

---

### 📚 ChapterWave

AI-Powered eBook Writing Platform

**Features**

* AI-assisted writing
* Markdown Editor
* Chapter Management
* PDF Export
* Author Dashboard
* Supabase Authentication

---

### 🍔 Fried & Crispy

Modern Restaurant Website

**Features**

* Responsive Design
* Menu Showcase
* Optimized Performance
* Deployed on Vercel

---

## 📈 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=himanshudev28\&show_icons=true\&theme=github_dark)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=himanshudev28\&layout=compact\&theme=github_dark)

![GitHub Streak](https://streak-stats.demolab.com?user=himanshudev28\&theme=github-dark-blue)

---

## 🏆 Achievements

* 🥈 Second Prize Winner – Robo Rumble
* 👨‍💻 Tech Committee Member
* 🎨 Contributed to UI/UX and Digital Content Projects
* 🤖 Completed Microsoft AI Azure Internship

---

## 📚 Currently Learning

```text
Backend Development
System Design
Advanced React Patterns
Data Structures & Algorithms
Cloud Technologies
```

---

## 🎯 2026 Goals

* Build SaaS Products
* Contribute to Open Source
* Master Backend Engineering
* Crack Top Software Development Roles
* Launch Personal Portfolio

---

## 🤝 Connect With Me

* LinkedIn: https://linkedin.com/in/himanshu2807
* GitHub: https://github.com/himanshudev28
* Email: [himanshu28071994@gmail.com](mailto:himanshu28071994@gmail.com)

---

### 💡 Favorite Quote

> "First solve the problem, then write the code."

⭐ Thanks for visiting my profile!

- ⚡ Fun fact: ...
-->
